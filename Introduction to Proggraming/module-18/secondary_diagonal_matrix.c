    ...
    else
    {
        ...
        for (int i = 0; i < row; i++)
        {
            for (int j = 0; j < col; j++)
            {
                ...
                // i + j != col - 1 -> not secondary diagonal line
                if ((i + j) != (col - 1) && arr[i][j] != 0) // is it outside of diagonal and its value is not 0
                {
        ...
        printf("Secondary Diagonal matrix bcz all test passed");
    }
